...
func uninstallCommands(plan Plan, purgeDriver bool) []string {
	...
	commands := []string{
		...
		powerShellCommand("$users = @(Get-Printer -ErrorAction Stop | Where-Object { $_.PortName -eq " + port + " }); $port = @(Get-PrinterPort -ErrorAction Stop | Where-Object { $_.Name -eq " + port + " }); if ($users.Count -gt 0) { 'Retained shared port' } elseif (-not " + port + ".StartsWith(" + powerShellString(managedPortPrefix) + ", [StringComparison]::OrdinalIgnoreCase)) { 'Retained external port' } elseif ($port.Count -gt 0) { $port | Remove-PrinterPort -ErrorAction Stop; 'Removed unused managed port' } else { 'Port already absent' }"),
	}
	if purgeDriver {
